...
class MainActivity : AppCompatActivity(), View.OnClickListener {
     ...
     * ML Kit ODT 설정
     * 이미지 준비: InputImage
     * 검사 프로그램 객체 : ObjectDetection.getClient(options)
     * 위의 두 객체 연결
     */
    private fun runObjectDetection(bitmap: Bitmap) {
        // InputImage 생성(Bitmap 객체로부터, 회전각도)
        ...
        val options = ObjectDetectorOptions.Builder()
            .setDetectorMode(ObjectDetectorOptions.SINGLE_IMAGE_MODE)     // 객체기가 짧은 지연 시간으로 진행
            .enableMultipleObjects()
            .enableClassification()                                 // Optional
            .build()

        // ObjectDetector 인스턴스를 가져옴
        /**
         * 객체 감지 및 분류는 비동기처리
         * - 이미지를 통해 검사 프로그램에 이미지 전송(process를 통해)
         * - 검사 프로그램은 복잡하게 작동
         * - 검사 프로그램에서는 콜백을 통해 결과 보고
         */
